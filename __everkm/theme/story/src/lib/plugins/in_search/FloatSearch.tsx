...
import {
  createLoaderEmitter,
  type StateChanger,
} from "../../widgets/infinite-loader";
import InfiniteLoader from "../../widgets/infinite-loader";
...
const SearchGlyph = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="ml-4 mr-2 shrink-0 text-muted-foreground" aria-hidden="true">
...
const EmptyGlyph = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" class="text-muted-foreground" aria-hidden="true">
...
const SearchResultItem = (props: {
  ...
  <a
    ...
    class={`hover:bg-state-hover block cursor-pointer space-y-1 px-4 py-3 no-underline ${
      props.isSelected ? "!bg-state-active" : ""
    }`}
    onClick={props.onClick}
  >
    <h2
      class="text-link inline font-medium underline-offset-2 hover:underline"
...
const KeyboardShortcuts = () => (
  <div class="text-text-tertiary hidden gap-3 text-sm md:flex">
    <div>
...
export default function FloatSearch(props: FloatSearchProps) {
  ...
  return (
    <div
      ...
      class={`fixed inset-0 z-[100] bg-black/20 backdrop-blur-sm dark:bg-black/40 ${visibilityClass()}`}
      onClick={(e) => {
        // 只有当点击的是背景层时才关闭
    ...
    >
      <div
        class="bg-surface text-text-primary max-w-screen-lg shadow-lg md:mx-4 md:my-8 md:rounded-[18px] md:border md:border-border lg:mx-auto"
      ...
      >
        ...
        >
          ...
          <input
            ...
            class="bg-surface text-text-primary min-w-12 flex-1 px-1 text-xl outline-0"
          />
          {hitItems().length > 0 && (
            <div
              class="hover:bg-state-hover text-text-secondary inline-flex flex-shrink-0 flex-grow-0 items-center rounded-lg p-2 text-lg opacity-60"
              onClick={(e) => {
                e.preventDefault()
                e.stopPropagation()
                handleClearSearch()
          ...
          <div
            class="border-border text-text-primary hover:bg-state-hover ml-2 flex h-full flex-shrink-0 flex-grow-0 cursor-pointer select-none items-center whitespace-nowrap border-l px-4"
            onClick={handleClose}
          >
            {t('cancel')}
          </div>
        ...
        {/* 空结果列表 */}
        {noResults() && (
          <div class="text-text-secondary flex flex-col items-center gap-4 py-8 text-center">
            <EmptyGlyph />
        ...
        {(hitItems().length > 0 || (wd().length > 0 && !loadFinish())) && (
          <div class="flex max-h-[calc(100dvh-3rem)] flex-col md:max-h-[calc(100dvh-7rem)]">
            ...
            {/* 底部说明 */}
            <div class="border-border bg-surface-muted flex items-center justify-between border-t px-4 py-2 md:rounded-b-[18px]">
